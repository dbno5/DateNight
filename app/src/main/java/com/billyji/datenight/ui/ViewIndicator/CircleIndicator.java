...
public class CircleIndicator extends LinearLayout {
    ...
    public void setViewPager(ViewFlipper viewPager) {
        mViewpager = viewPager;
        if (mViewpager != null) {
            mLastPosition = -1;
            createIndicators();
            mViewpager.removeOnLayoutChangeListener(mInternalPageChangeListener);
            mViewpager.addOnLayoutChangeListener(mInternalPageChangeListener);
            mInternalPageChangeListener.onLayoutChange(null, mViewpager.getDisplayedChild(), 0, 0, 0, 0, 0, 0, 0);
        }
    }

    private final OnLayoutChangeListener mInternalPageChangeListener = new OnLayoutChangeListener() {
    ...
    private DataSetObserver mInternalDataSetObserver = new DataSetObserver() {
        @Override public void onChanged() {
            ...
            int newCount = mViewpager.getChildCount();
            int currentCount = getChildCount();

            if (newCount == currentCount) {  // No change
                return;
            } else if (mLastPosition < newCount) {
                mLastPosition = mViewpager.getDisplayedChild();
            } else {
                mLastPosition = -1;
            }
     ...
     */
    @Deprecated public void setOnPageChangeListener(OnLayoutChangeListener onPageChangeListener) {
        if (mViewpager == null) {
            throw new NullPointerException("can not find Viewpager , setViewPager first");
        }
        mViewpager.removeOnLayoutChangeListener(onPageChangeListener);
        mViewpager.addOnLayoutChangeListener(onPageChangeListener);
    }
    ...
    private void createIndicators() {
        ...
        int currentItem = mViewpager.getDisplayedChild();
